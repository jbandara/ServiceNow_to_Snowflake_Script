...
def fetch_and_process_incidents(session, incident_fields, group_filter):
    all_incidents = []
    offset = 0
    limit = 3000
...
def lambda_handler(event, context):
    ...
    query = """
    SELECT COALESCE(MAX(sys_updated_on),'2026-01-01 00:00:00')
    ...
    all_incidents = fetch_and_process_incidents(session, incident_fields, group_filter)
