...
CHANGE_API = f"{BASE_URL}/api/now/table/change_request"
...
#Changes Extraction Process
def fetch_and_process_changes(session, change_fields, group_filter, last_run_time):

    params = {
        "sysparm_fields": change_fields,
        "sysparm_limit": "10000",
        "sysparm_query":
            f"assignment_group.nameIN{group_filter}"
            f"^sys_updated_on>={last_run_time}"
    }

    print("Getting Change Requests from ServiceNow...")
...
def lambda_handler(event, context):
    ...
    print("======================================")
    print("CHANGE REQUEST EXTRACTION STARTED")
    print("======================================")
    ...
    FROM KSFTA.DDRPF.SERVICENOW_CHANGES
    """

    cursor.execute(change_query)

    last_change_run = cursor.fetchone()[0]

    print(
        f"Last Change Run Time = "
        f"{last_change_run}"
    )
    ...
    change_fields = ",".join([
        ...
        "u_outage_required_",
        "sys_id",
        "u_approved_date_time",
        "reason"
    ])

    all_changes = fetch_and_process_changes(
        session,
        change_fields,
        group_filter,
        last_change_run
    ...
    print("OPENED_AT sample:")
    print(change_df["opened_at"].head(20).tolist())

    print("SYS_UPDATED_ON sample:")
    print(change_df["sys_updated_on"].head(20).tolist())

    print("U_APPROVED_DATE_TIME sample:")
    print(change_df["u_approved_date_time"].head(20).tolist())
    ...
    if not change_df.empty:
        ...
        for col in change_target_columns:
        ...
        with snowflake_conn.cursor() as source_cursor:
            ...
            source_cursor.execute(
                f'''
                CREATE TEMP TABLE
                "{target_database}"."{target_schema}"."{change_stage_table}"
                LIKE
                "{target_database}"."{target_schema}"."SERVICENOW_CHANGES"
                '''
            )
        ...
        if not success:
            raise RuntimeError(
                "Failed loading change requests"
            )
        ...
        merge_sql = f'''
            MERGE INTO
            "{target_database}"."{target_schema}"."SERVICENOW_CHANGES" tgt

            USING
            "{target_database}"."{target_schema}"."{change_stage_table}" src
